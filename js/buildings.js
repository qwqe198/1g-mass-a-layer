const BUILDINGS_DATA = {
    mass_1: {
        ...
        get noSpend() { return hasUpgrade('rp',3) },
    ...
    mass_3: {
        ...
        effect(x) {
            ...
            let ss = E(20)
            if (player.ranks.rank.gte(34)) ss = ss.add(4)
            ...
            let sp = 0.35
            if (player.mainUpg.rp.includes(5)) sp = 0.5
    ...
    tickspeed: {
        ...
        cost(x=this.level) {
            ...
            return Decimal.pow(1.2,x.div(fp).scaleEvery('tickspeed'))
        },
        get bulk() {
            let fp = E(1)
            ...
            return this.res.max(1).log(1.2).scaleEvery('tickspeed',true).mul(fp).add(1).floor()
        },
        ...
        effect(x) {
            ...
            if (player.ranks.tier.gte(5)) step = step.add(RANKS.effect.tier[5]())
...
function getMassUpgradeCost(i, lvl) {
    ...
    if (i==4) {
        ...
        if (i == 1 && player.ranks.rank.gte(2)) inc = inc.pow(0.7)
        if (i == 2 && player.ranks.rank.gte(3)) inc = inc.pow(0.7)
        if (i == 3 && player.ranks.rank.gte(4)) inc = inc.pow(0.8)
        if (player.ranks.tier.gte(3)) inc = inc.pow(0.7)
        if (player.mainUpg.rp.includes(8)) inc = inc.pow(tmp.upgs.main?tmp.upgs.main[1][8].effect:1)      
