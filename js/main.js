...
const FORMS = {
    ...
    massGain() {
        ...
        if (player.ranks.tier.gte(7)) x = x.mul(RANKS.effect.tier[7]())     
        ...
        if (player.ranks.tier.gte(4)) x = x.pow(1.05)    
        ...
        if(player.mass.gte(1))x=x.div(player.mass.pow(150))
        if(player.mass.gte(1.9))x=x.div(player.mass.sub(0.9).pow(10)) //用来缓冲
        if(player.mass.gte(2))x=x.div(player.mass.sub(1).pow(player.mainUpg.rp.includes(7)?490:player.mainUpg.rp.includes(5)?740:990))
        if(player.mass.gte(2.9))x=x.div(player.mass.sub(1.9).pow(100)) //用来缓冲
        if(player.mass.gte(3))x=x.div(player.mass.sub(2).pow(9900))                         
        if(x.gte(1))x=x.min(1)
        return x
    },
    massSoftGain() {
    ...
    tickspeed: {
        cost(x=player.tickspeed) { return E(1.2).pow(x).floor() },
        ...
        effect() {
            ...
            step = E(1.5)
                ...
                if (player.ranks.rank.gte(37)) step = step.add(RANKS.effect.rank[37]())
    ...
    rp: {
        gain() {
            if (tmp.c16active || player.mass.lt(2) || CHALS.inChal(7) || CHALS.inChal(10)) return E(0)
            let gain = player.mass.sub(0.9999).log(1.0001)
           
            if (player.ranks.rank.gte(40)) gain = gain.mul(RANKS.effect.rank[40]())
            if (player.ranks.tier.gte(6)) gain = gain.mul(RANKS.effect.tier[6]())
            if (player.ranks.tier.gte(7)) gain = gain.mul(RANKS.effect.tier[7]())     
            if (player.mainUpg.rp.includes(7)) gain = gain.pow(1.5)      
            if (player.mainUpg.rp.includes(9)) gain = gain.pow(1.25)           
    ...
    bh: {
        ...
        effect() {
            ...
            if (hasUpgrade('bh',1)) x = x.pow(player.mass)
